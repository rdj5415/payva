...
from auditpulse_mvp.database.models import Transaction, Anomaly, AnomalyType, FeedbackType
...
class TransactionAdmin:
    ...
    async def get_all(
        ...
        conditions = [Transaction.tenant_id == tenant_id, Transaction.is_deleted == False]
    ...
    async def get_stats(
        ...
        # Sum total amount
        sum_stmt = select(func.sum(Transaction.amount)).select_from(Transaction).where(and_(*conditions))
        ...
        # Get average amount
        avg_stmt = select(func.avg(Transaction.amount)).select_from(Transaction).where(and_(*conditions))
...
class AnomalyAdmin:
    ...
    async def get_all(
        ...
        stmt = (
            stmt.order_by(Anomaly.created_at.desc())
            .offset(skip)
            .limit(limit)
        )
    ...
    async def get_stats(
        ...
        if start_date or end_date:
            base_query = base_query.join(Transaction, Anomaly.transaction_id == Transaction.id)
...
class ModelAdmin:
    ...
    async def get_performance_history(
        ...
        # Add pagination and ordering
        stmt = (
            stmt.order_by(ModelPerformance.recorded_at.desc())
            .limit(limit)
        )
    ...
    async def export_model_config(self, version_id: UUID4) -> Dict[str, Any]:
        ...
        config = {
            ...
            "performance_metrics": latest_performance.metrics if latest_performance else None,
            "last_evaluated": latest_performance.recorded_at.isoformat() if latest_performance else None,
        }
    ...
    async def rename_model_version(
        ...
        if existing_version and existing_version.id != version_id:
            logger.error(f"Cannot rename model version: {new_version} already exists for type {model_version.model_type}")
            return None
        ...
        logger.info(f"Renamed model version from {old_version} to {new_version} for {model_version.model_type}")
        return model_version 