...
class NotificationService:
    ...
    async def send_anomaly_notification(
        ...
        try:
            ...
            if not users:
                logger.warning(
                    f"No notification recipients found for tenant {anomaly.tenant_id}"
                )
                return {
                    "email": NotificationStatus.FAILED,
                    "slack": NotificationStatus.FAILED,
            ...
            if await self.webhook_provider.is_configured():
                webhook_recipients = [
                    user.webhook_url
                    for user in users
                    if hasattr(user, "webhook_notifications")
                    and user.webhook_notifications
            ...
            logger.exception(
                f"Error sending notifications for anomaly {anomaly_id}: {str(e)}"
            )
            return {
                "email": NotificationStatus.FAILED,
                "slack": NotificationStatus.FAILED,
    ...
    async def notify_high_risk_anomalies(self, db: AsyncSession) -> int:
        ...
        try:
            ...
            for anomaly in anomalies:
                ...
                if any(
                    status == NotificationStatus.SENT for status in results.values()
                ):
                    notification_count += 1

            logger.info(
                f"Sent notifications for {notification_count} high-risk anomalies"
            )
            return notification_count

        except Exception as e:
            logger.exception(
                f"Error sending notifications for high-risk anomalies: {str(e)}"
            )
            return 0
    ...
    def _create_notification_payload(self, anomaly: Anomaly) -> NotificationPayload:
        ...
        dashboard_url = (
            ...
            else f"/dashboard/anomalies/{anomaly.id}"
        )
        ...
        payload = NotificationPayload(
            ...
            metadata={
                "transaction_type": getattr(
                    anomaly.transaction, "transaction_type", "unknown"
                ),
