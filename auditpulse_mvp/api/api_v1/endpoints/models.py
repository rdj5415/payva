...
class ModelVersionCreate(BaseModel):
    ...
    model_type: str = Field(
        ..., description="Type of model (e.g., 'anomaly_detection')"
    )
...
class ModelPredictionRequest(BaseModel):
    ...
    version: Optional[str] = Field(
        None, description="Specific version to use (defaults to active)"
    )
...
@router.post(
    "/versions/{model_type}/{version}/activate", response_model=ModelVersionResponse
)
...
@router.post(
    "/versions/{model_type}/{version}/rollback", response_model=ModelVersionResponse
)
...
async def validate_model(
    ...
    try:
        ...
        version = await model_manager.get_version(
            validation.model_type, validation.version
        )
        if not version:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
        ...
        predictions = [
            random.choice([0, 1]) for _ in range(len(validation.ground_truth))
        ]
        ...
        try:
            metrics = {
                ...
                "precision": precision_score(
                    validation.ground_truth, predictions, zero_division=0
                ),
                "recall": recall_score(
                    validation.ground_truth, predictions, zero_division=0
                ),
...
async def predict(
    ...
    try:
        ...
        version_obj = None
        if request.version:
            version_obj = await model_manager.get_version(
...
async def model_health_check(
    ...
    version: Optional[str] = Query(
        None, description="Optional version identifier (defaults to active)"
    ),
    model_manager: ModelManager = Depends(),
    current_user: User = Depends(get_current_user),
) -> Dict[str, Any]:
    ...
    try:
        ...
        performance = await model_manager.get_performance_history(
            model_type=model_type, version=version_obj.version, limit=1
        )
        ...
        return {
            ...
            "last_performance_check": (
                performance[0].recorded_at.isoformat() if performance else None
            ),
