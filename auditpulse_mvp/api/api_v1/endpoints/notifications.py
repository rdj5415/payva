...
from auditpulse_mvp.database.models.notification import Notification, NotificationTemplate, NotificationDeliveryAttempt
from auditpulse_mvp.notifications.notification_manager import NotificationManager, NotificationRecipient, NotificationRequest, NotificationPriority
...
class TemplateCreate(BaseModel):
    ...
    placeholders: Optional[Dict[str, str]] = Field(None, description="Template placeholder descriptions")
...
class NotificationCreate(BaseModel):
    ...
    template_data: Optional[Dict[str, Any]] = Field({}, description="Data for template rendering")
    priority: NotificationPriority = Field(NotificationPriority.MEDIUM, description="Notification priority")
    channels: Optional[List[str]] = Field(None, description="Channels to send notification through")
...
class NotificationBatchCreate(BaseModel):
    ...
    max_concurrent: Optional[int] = Field(10, description="Maximum number of concurrent notification processing tasks")
...
class NotificationPreferencesUpdate(BaseModel):
    ...
    channels: Dict[str, List[str]] = Field(..., description="Channel preferences for different notification types")

    @validator("channels")
    def validate_channels(cls, v):
        """Validate that channels are valid."""
        valid_types = ["anomaly_detection", "system_alert", "model_performance", "account_security", "scheduled_reports"]
...
async def create_template(
    ...
    if not current_user.is_superuser and not current_user.has_permission("manage_templates"):
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Could not create template: {str(e)}")
...
async def create_notification(
    ...
    if not template:
        raise HTTPException(status_code=404, detail=f"Template '{notification.template_id}' not found")
    ...
    try:
        ...
        db_notification = await db.query(Notification).filter(
        ...
        raise HTTPException(status_code=400, detail=f"Could not create notification: {str(e)}")
...
async def create_batch_notifications(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, 
            detail=f"Could not create batch notifications: {str(e)}"
        )
...
async def get_notification(
    ...
    if not notification:
        ...
        if current_user.is_superuser or current_user.has_permission("view_all_notifications"):
            notification = await db.query(Notification).filter(
                Notification.id == notification_id
            ).first()
    ...
    delivery_attempts = await db.query(NotificationDeliveryAttempt).filter(
