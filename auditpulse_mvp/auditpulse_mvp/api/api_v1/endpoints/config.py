...
class RiskEngineSettings(BaseModel):
    ...
    def weights_must_sum_to_one(cls, v, values):
        ...
        if (
            "rules_score_weight" in values
            and abs(values["rules_score_weight"] + v - 1.0) > 0.001
        ):
...
class RulesConfiguration(BaseModel):
    ...
    large_transaction: RuleSettings = Field(
        default_factory=lambda: RuleSettings(
            enabled=True, threshold=10000.0, parameters={"scale_factor": 1.0}
        )
    )
    unusual_counterparty: RuleSettings = Field(
        default_factory=lambda: RuleSettings(
            enabled=True, parameters={"min_frequency": 3}
        )
    )
    weekend_transaction: RuleSettings = Field(
        default_factory=lambda: RuleSettings(
            enabled=True, parameters={"score_multiplier": 1.5}
        )
    )
    irregular_amount: RuleSettings = Field(
        default_factory=lambda: RuleSettings(
            enabled=True, threshold=2.0, parameters={"std_dev_threshold": 2.0}
        )
    )
    round_number_transaction: RuleSettings = Field(
        default_factory=lambda: RuleSettings(
            enabled=True, parameters={"score_multiplier": 1.2}
        )
    )
...
def get_preset_configuration(level: SensitivityLevel) -> SensitivityConfiguration:
    ...
    if level == SensitivityLevel.LOW:
        return SensitivityConfiguration(
            ...
            rules=RulesConfiguration(
                large_transaction=RuleSettings(
                    enabled=True, threshold=20000.0, parameters={"scale_factor": 0.8}
                ),
                unusual_counterparty=RuleSettings(
                    enabled=True, parameters={"min_frequency": 2}
                ),
                weekend_transaction=RuleSettings(
                    enabled=False,
                ),
                irregular_amount=RuleSettings(
                    enabled=True, threshold=3.0, parameters={"std_dev_threshold": 3.0}
                ),
        ...
        return SensitivityConfiguration(
            ...
            rules=RulesConfiguration(
                large_transaction=RuleSettings(
                    enabled=True, threshold=5000.0, parameters={"scale_factor": 1.2}
                ),
                unusual_counterparty=RuleSettings(
                    enabled=True, parameters={"min_frequency": 1}
                ),
                weekend_transaction=RuleSettings(
                    enabled=True, parameters={"score_multiplier": 2.0}
                ),
                irregular_amount=RuleSettings(
                    enabled=True, threshold=1.5, parameters={"std_dev_threshold": 1.5}
                ),
                round_number_transaction=RuleSettings(
                    enabled=True, parameters={"score_multiplier": 1.5}
                ),
...
async def get_tenant_sensitivity_config(
    ...
    if config and config.value:
        try:
            ...
            sensitivity_level = config_data.get(
                "sensitivity_level", SensitivityLevel.MEDIUM
            )
