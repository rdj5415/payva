...
class MLAnomalyDetector:
    ...
    async def process_transaction(
        ...
        try:
            ...
            if e.status_code == status.HTTP_404_NOT_FOUND:
                ...
                logger.info(
                    f"No ML model found for tenant {tenant_id}, using rules only"
                )
            else:
    ...
    async def process_transactions(
        ...
        for tenant_id, tenant_txns in transactions_by_tenant.items():
            ...
            ml_scores = {}
            try:
                ml_scores = await self.ml_engine.batch_score_transactions(
                    tenant_id, tenant_txns
                )
            except HTTPException as e:
                if e.status_code == status.HTTP_404_NOT_FOUND:
                    ...
                    logger.error(
                        f"Error batch scoring transactions with ML: {e.detail}"
                    )
                    ml_scores = {txn.id: 0.5 for txn in tenant_txns}
            except Exception as e:
                # Log error but continue with neutral scores
                logger.exception(
                    f"Unexpected error batch scoring transactions with ML: {e}"
                )
                ml_scores = {txn.id: 0.5 for txn in tenant_txns}
            ...
            for txn in tenant_txns:
                ml_score = ml_scores.get(
                    txn.id, 0.5
                )  # Default to neutral if not in results
                ml_anomaly_detected = ml_score >= self.ml_score_threshold
                ...
                if check_rules and not anomaly:
                    try:
                        ...
                        logger.exception(
                            f"Error evaluating transaction with rules engine: {e}"
                        )
