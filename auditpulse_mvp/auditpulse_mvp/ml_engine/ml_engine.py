...
class MLEngine:
    ...
    def _preprocess_data(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, StandardScaler]:
        ...
        numeric_cols = [col for col in processed_df.columns if col in NUMERICAL_FEATURES]
    ...
    async def train_model(
        ...
        try:
            ...
            model_info = {
                ...
                "feature_importance": self._calculate_feature_importance(model, processed_df.columns),
    ...
    async def batch_score_transactions(
        ...
        try:
            ...
            if "404" in str(e):
                logger.warning(f"No model found for tenant {tenant_id}, returning neutral scores")
                return {txn.id: 0.5 for txn in transactions}
            # Re-raise other HTTP exceptions
            raise
        except Exception as e:
            logger.exception(f"Error batch scoring transactions for tenant {tenant_id}: {e}")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def delete_model(self, tenant_id: uuid.UUID, model_filename: str) -> bool:
        ...
        try:
            ...
            logger.exception(f"Error deleting model {model_filename} for tenant {tenant_id}: {e}")
            return False
            ...
            for tenant in tenants:
                try:
                    ...
                    if not tenant.enable_ml_engine:
                        logger.info(f"ML engine disabled for tenant {tenant.id}, skipping retraining")
                        results[tenant.id] = {"status": "skipped", "reason": "ML engine disabled"}
                        continue
                    ...
                    logger.warning(f"Error retraining model for tenant {tenant.id}: {e.detail}")
                except Exception as e:
                    results[tenant.id] = {"status": "error", "message": str(e)}
                    logger.error(f"Unexpected error retraining model for tenant {tenant.id}: {e}")
