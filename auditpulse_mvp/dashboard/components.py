...
def render_anomaly_details(anomaly: Anomaly) -> None:
    ...
    feedback = st.radio(
        "Is this anomaly valid?", ["Yes", "No", "Not Sure"], horizontal=True
    )
...
def render_risk_metrics(
    transactions: List[Transaction], anomalies: List[Anomaly]
) -> None:
    ...
    with col1:
        st.metric("Risk Ratio", f"{risk_ratio:.1%}", delta=f"{risk_ratio - 0.1:.1%}")
    with col2:
        st.metric(
            "Anomaly Amount",
            f"${anomaly_amount:,.2f}",
    ...
    df = pd.DataFrame(
        [
            {"date": a.created_at, "risk_score": a.risk_score, "type": a.type}
            for a in anomalies
        ]
    )
    ...
    if not df.empty:
        fig = px.line(
            df, x="date", y="risk_score", color="type", title="Risk Score Trend"
        )
