...
class MLEngine:
    ...
    def _get_model_path(self, tenant_id: uuid.UUID, timestamp: Optional[datetime.datetime] = None) -> Path:
    ...
    async def load_model(self, tenant_id: uuid.UUID) -> Dict[str, Any]:
        ...
        if not model_path:
            ...
            try:
                ...
                raise ValueError(f"No model exists for tenant {tenant_id} and insufficient data to train one")
    ...
    def _extract_features_from_transaction(
        ...
        if any(f.startswith("account_") for f in feature_list) and "source_account_id" in df:
    ...
    async def score(self, tenant_id: uuid.UUID, transaction: Transaction) -> float:
        ...
        try:
            ...
            logger.error(f"Error scoring transaction {transaction.id} for tenant {tenant_id}: {e}")
            # Return a moderate score in case of error
            return 0.5
    
    async def batch_train_models(self, tenant_ids: Optional[List[uuid.UUID]] = None) -> Dict[str, Any]:
        ...
        logger.info(f"Batch training completed. Success: {len(results['success'])}, Error: {len(results['error'])}")
        return results
