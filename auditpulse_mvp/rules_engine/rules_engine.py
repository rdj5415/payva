...
class Rule:
    ...
    async def evaluate(self, transaction: Transaction, db: AsyncSession) -> Tuple[float, str]:
...
class AmountThresholdRule(Rule):
    ...
    async def evaluate(self, transaction: Transaction, db: AsyncSession) -> Tuple[float, str]:
        ...
        if transaction.amount > self.threshold:
            ...
            return score, f"Transaction amount {transaction.amount} exceeds threshold {self.threshold}"
        return 0.0, ""
...
class StatisticalOutlierRule(Rule):
    ...
    def __init__(self, std_dev_threshold: float = 3.0, lookback_days: int = 30, weight: float = 1.0):
    ...
    async def evaluate(self, transaction: Transaction, db: AsyncSession) -> Tuple[float, str]:
        ...
        if z_score > self.std_dev_threshold:
            ...
            return score, f"Transaction amount {transaction.amount} is {z_score:.1f}σ from mean {mean:.2f}"
        return 0.0, ""
