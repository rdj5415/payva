...
class SensitivityConfiguration(BaseModel):
    ...
    sensitivity_level: SensitivityLevel = Field(..., description="Overall sensitivity level")
...
class SensitivityConfigResponse(BaseModel):
    ...
    id: str = Field(..., description="The unique identifier for the sensitivity configuration")
    rule_type: str = Field(..., description="The type of rule (e.g., 'amount_threshold', 'frequency_threshold')")
...
class ConfigUpdateResult(BaseModel):
    ...
    sensitivity_before: float = Field(..., description="The sensitivity value before the update")
    sensitivity_after: float = Field(..., description="The sensitivity value after the update")
...
class RetrainingStatusResult(BaseModel):
    ...
    message: Optional[str] = Field(None, description="Additional message about the retraining process")
...
class SingleTenantLearningResult(BaseModel):
    ...
    tenant_id: str = Field(..., description="The tenant ID")
    feedback_analysis: Optional[Dict[str, Any]] = Field(None, description="Analysis of feedback data")
    config_updates: Optional[Dict[str, Any]] = Field(None, description="Updates to sensitivity configurations")
    retraining_status: Optional[Dict[str, Any]] = Field(None, description="Status of model retraining")
...
class LearningTriggerResponse(BaseModel):
    ...
    status: str = Field(..., description="Status of the learning process (success, skipped, error)")
    processed_count: int = Field(..., description="Number of anomalies processed")
    false_positive_rate: Optional[float] = Field(None, description="Rate of false positives in processed anomalies")
    true_positive_rate: Optional[float] = Field(None, description="Rate of true positives in processed anomalies")
    config_updated: Optional[bool] = Field(None, description="Whether the sensitivity configuration was updated")
    model_retrained: Optional[bool] = Field(None, description="Whether the ML model was retrained")
    model_info: Optional[Dict[str, Any]] = Field(None, description="Information about the retrained model, if any")
...
class ScheduledLearning(BaseModel):
    ...
    frequency: str = Field("daily", description="Frequency of learning (daily, weekly, etc.)")
...
class LastLearningRun(BaseModel):
    ...
    status: str = Field(..., description="Status of the learning run (success, skipped, error)")
    processed_count: int = Field(..., description="Number of anomalies processed")
    rules_updated: Optional[int] = Field(None, description="Number of rules updated")
    models_updated: Optional[int] = Field(None, description="Number of ML models updated")
    false_positive_rate: Optional[float] = Field(None, description="Overall false positive rate")
    true_positive_rate: Optional[float] = Field(None, description="Overall true positive rate")
    error: Optional[str] = Field(None, description="Error details if status is error")
...
class LearningStatus(BaseModel):
    ...
    last_learning_run: Optional[LastLearningRun] = Field(None, description="Information about the last learning run")
    scheduled_learning: ScheduledLearning = Field(..., description="Scheduled learning configuration")
...
class FeedbackTypeCounts(BaseModel):
    ...
    needs_investigation: int = Field(0, description="Number of anomalies marked for investigation")
...
class LearningAdjustments(BaseModel):
    ...
    rules_adjusted: int = Field(0, description="Number of rules adjusted based on feedback")
    ml_models_adjusted: int = Field(0, description="Number of ML models adjusted based on feedback")
    last_adjustment_date: Optional[str] = Field(None, description="ISO timestamp of last adjustment")
...
class FeedbackStatistics(BaseModel):
    ...
    rules_based: FeedbackTypeCounts = Field(..., description="Statistics for rules-based anomalies")
    ml_based: FeedbackTypeCounts = Field(..., description="Statistics for ML-based anomalies")
    combined: FeedbackTypeCounts = Field(..., description="Combined statistics for all anomalies")
    learning_adjustments: LearningAdjustments = Field(..., description="Information about learning-based adjustments")
...
class LearningScheduleUpdate(BaseModel):
    ...
    hour: int = Field(..., ge=0, le=23, description="Hour of day to run learning (0-23)")
    minute: int = Field(..., ge=0, le=59, description="Minute of hour to run learning (0-59)")


class SensitivityUpdateResponse(BaseModel):
    """Response model for sensitivity update."""
    
    status: str = Field(..., description="Status of the update (success, skipped, error)")
    message: Optional[str] = Field(None, description="Human-readable message about the update")
    update_applied: bool = Field(..., description="Whether the sensitivity was updated")
    previous_level: Optional[str] = Field(None, description="Previous sensitivity level")
    new_level: Optional[str] = Field(None, description="New sensitivity level")
    false_positive_rate: Optional[float] = Field(None, description="False positive rate that led to the update")
    true_positive_rate: Optional[float] = Field(None, description="True positive rate that led to the update")
    reason: Optional[str] = Field(None, description="Reason for skipping or error") 