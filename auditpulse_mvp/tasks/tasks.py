...
async def sync_data_task(tenant_ids: Optional[List[str]] = None):
    ...
    """
    logger.info(f"Starting transaction data sync task for tenants: {tenant_ids or 'all'}")
...
async def retrain_models_task(tenant_ids: Optional[List[str]] = None):
    ...
    async with get_db_session() as db:
        ...
        logger.info(f"ML model retraining results: {result['success']} successful, {result['error']} failed")
    ...
    async with get_db_session() as db:
        ...
        for tenant in tenants:
            try:
                ...
                logger.info(f"Processing {len(transactions)} transactions for tenant {tenant.id}")
                
                # Process each transaction
                anomalies_found = 0
                for transaction in transactions:
                    anomaly = await risk_engine.evaluate_and_store(tenant.id, transaction)
...
async def cleanup_old_data_task(retention_days: int = 730):  # Default: 2 years
    ...
    async with get_db_session() as db:
        try:
            ...
            deleted_count = await db.query(Transaction).filter(
                Transaction.transaction_date < cutoff_date,
                Transaction.is_deleted == False
            ).update(
                {"is_deleted": True}
