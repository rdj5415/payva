...
async def test_submit_anomaly_feedback():
    ...
    assert feedback_call[0][0] == f"http://localhost:8000/api/v1/anomalies/{anomaly_id}/feedback"
    ...
    assert notification_call[0][0] == f"http://localhost:8000/api/v1/alerts/send/{anomaly_id}"
...
def test_mock_integration():
    ...
    with patch("streamlit.session_state", {"authenticated": True}), \
        ...
        with patch.dict("sys.modules", {"streamlit": MagicMock()}):
            ...
            mock_loop_instance.run_until_complete.return_value = (mock_anomalies_data, len(mock_anomalies_data))
