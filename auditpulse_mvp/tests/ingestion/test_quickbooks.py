...
@pytest.fixture
async def mock_tenant(db_session: AsyncSession, mock_tenant_id: uuid.UUID, mock_qb_settings: dict) -> Tenant:
...
async def test_qb_client_init():
    ...
    with patch("auditpulse_mvp.ingestion.quickbooks.settings") as mock_settings:
        ...
        mock_settings.QUICKBOOKS_CLIENT_SECRET.get_secret_value.return_value = "test_secret"
...
async def test_qb_client_get_authorization_url():
    ...
    with patch("auditpulse_mvp.ingestion.quickbooks.settings") as mock_settings, \
         patch("auditpulse_mvp.ingestion.quickbooks.AuthClient") as MockAuthClient:
        mock_settings.QUICKBOOKS_CLIENT_ID = "test_client_id"
        mock_settings.QUICKBOOKS_CLIENT_SECRET.get_secret_value.return_value = "test_secret"
        mock_settings.QUICKBOOKS_REDIRECT_URI = "https://example.com/callback"
        
        mock_auth_client = MagicMock()
        mock_auth_client.get_authorization_url.return_value = "https://oauth.platform.intuit.com/oauth2/v1/authorize?..."
...
async def test_qb_client_fetch_transactions_success():
    ...
    with patch("auditpulse_mvp.ingestion.quickbooks.settings") as mock_settings, \
        ...
        mock_response.json.return_value = {
            "QueryResponse": {
                "Purchase": [{"Id": "123", "TotalAmt": 100.00}]
            }
        }
        ...
        assert "2023-01-01" in mock_async_client.__aenter__.return_value.post.call_args[1]["data"]
        assert "2023-02-01" in mock_async_client.__aenter__.return_value.post.call_args[1]["data"]
...
async def test_qb_client_fetch_transactions_http_error():
    ...
    with patch("auditpulse_mvp.ingestion.quickbooks.settings") as mock_settings, \
         patch("httpx.AsyncClient") as MockAsyncClient, \
         patch.object(QuickBooksClient, "refresh_access_token") as mock_refresh:
        mock_settings.QUICKBOOKS_CLIENT_ID = "test_client_id"
        mock_settings.QUICKBOOKS_CLIENT_SECRET.get_secret_value.return_value = "test_secret"
        mock_settings.QUICKBOOKS_REDIRECT_URI = "https://example.com/callback"
...
async def test_qb_client_refresh_access_token():
    ...
    with patch("auditpulse_mvp.ingestion.quickbooks.settings") as mock_settings, \
        ...
        assert tokens["refresh_token"] == "new_refresh_token"
        assert tokens["realm_id"] == "12345"
        mock_auth_client.refresh.assert_called_once_with(refresh_token="old_refresh_token")
...
async def test_qb_service_get_client_for_tenant_without_qb(
    ...
    assert f"QuickBooks not configured for tenant {mock_tenant_id}" in str(excinfo.value.detail)
...
async def test_qb_service_sync_transactions(
    ...
    with patch.object(QuickBooksClient, "fetch_transactions") as mock_fetch:
        ...
        with patch.object(QuickBooksService, "get_client_for_tenant") as mock_get_client:
...
@pytest.mark.asyncio
async def test_handle_quickbooks_webhook_no_events(db_session: AsyncSession, mock_tenant: Tenant):
