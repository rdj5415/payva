...
async def test_send_notification(
    notification_manager, mock_db_session, mock_task_manager, monkeypatch
):
    """Test sending a notification."""
    # Setup
    monkeypatch.setattr(
        "auditpulse_mvp.database.session.get_db",
        AsyncMock(return_value=mock_db_session),
    )
    ...
    mock_db_session.refresh.side_effect = lambda x: setattr(
        x, "id", mock_notification.id
    )
...
@pytest.mark.asyncio
async def test_send_notification_with_schedule(
    notification_manager, mock_db_session, mock_task_manager, monkeypatch
):
...
@pytest.mark.asyncio
async def test_send_batch_notification(
    notification_manager, mock_db_session, mock_task_manager, monkeypatch
):
...
async def test_get_notification_status(
    ...
    mock_notification.to_dict = MagicMock(
        return_value={
    ...
    result = await notification_manager.get_notification_status(
        str(mock_notification.id)
    )
