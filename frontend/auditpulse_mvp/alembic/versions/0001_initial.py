...
def upgrade() -> None:
    ...
    # Create enum types
    op.execute("CREATE TYPE datasource AS ENUM ('quickbooks', 'plaid', 'manual', 'netsuite', 'csv')")
    op.execute("CREATE TYPE anomalytype AS ENUM ('rules_based', 'ml_based', 'manual')")
    op.execute("CREATE TYPE feedbacktype AS ENUM ('true_positive', 'false_positive', 'ignore', 'needs_review')")
    ...
    op.create_table(
        ...
        sa.Column('subscription_tier', sa.String(50), nullable=False, default='standard'),
        sa.Column('config', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
        sa.Column('quickbooks_settings', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
        sa.Column('plaid_settings', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
        sa.Column('enable_ml_engine', sa.Boolean(), nullable=False, default=True),
        sa.Column('enable_gpt_explanations', sa.Boolean(), nullable=False, default=True),
    ...
    op.create_table(
        ...
        sa.Column('source', sa.Enum('quickbooks', 'plaid', 'manual', 'netsuite', 'csv', name='datasource'), nullable=False),
        ...
        sa.ForeignKeyConstraint(['tenant_id'], ['tenants.id'], ),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('tenant_id', 'transaction_id', 'source', name='uq_transaction_per_tenant_source')
    )
    ...
    op.create_table(
        ...
        sa.Column('anomaly_type', sa.Enum('rules_based', 'ml_based', 'manual', name='anomalytype'), nullable=False),
        sa.Column('rule_name', sa.String(255), nullable=True),
        sa.Column('risk_score', sa.Float(), nullable=False),
        sa.Column('confidence_score', sa.Float(), nullable=False),
        sa.Column('description', sa.Text(), nullable=False),
        sa.Column('ai_explanation', sa.Text(), nullable=True),
        sa.Column('feedback', sa.Enum('true_positive', 'false_positive', 'ignore', 'needs_review', name='feedbacktype'), nullable=True),
