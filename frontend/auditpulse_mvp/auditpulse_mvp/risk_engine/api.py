...
async def get_risk_config(
    ...
    return RiskConfigResponse(
        ...
        tenant_id=tenant_id,
        updated_at=datetime.fromisoformat(tenant_config.get("updated_at", datetime.now().isoformat()))
        if "updated_at" in tenant_config else None
    )
...
async def update_risk_weights(
    ...
    try:
        ...
        risk_weights = RiskWeights(
            ...
            rules_weight=weights.rules_weight
        )
    ...
    tenant.metadata["risk_config"]["weights"]["rules_weight"] = risk_weights.rules_weight
...
async def update_risk_sensitivity(
    ...
    tenant.metadata["risk_config"]["sensitivity"]["low_threshold"] = risk_sensitivity.low_threshold
    tenant.metadata["risk_config"]["sensitivity"]["medium_threshold"] = risk_sensitivity.medium_threshold
    tenant.metadata["risk_config"]["sensitivity"]["high_threshold"] = risk_sensitivity.high_threshold
...
async def rescore_anomaly(
    ...
    generate_explanation: bool = Query(False, description="Whether to generate an explanation"),
    db_session: AsyncSession = Depends(get_db_session),
    ...
    anomaly_dict = {
        ...
        "transaction_amount": float(anomaly.transaction.amount) if anomaly.transaction else 0.0,
        "transaction_type": anomaly.transaction.transaction_type if anomaly.transaction else "unknown",
        "merchant_name": anomaly.transaction.merchant_name if anomaly.transaction else "unknown",
        "transaction_date": anomaly.transaction.transaction_date.isoformat() if anomaly.transaction else datetime.now().isoformat(),
...
async def rescore_tenant_anomalies(
    tenant_id: str,
    days: int = Query(settings.risk_rescoring_days, description="Number of days of anomalies to rescore"),
    limit: int = Query(100, description="Maximum number of anomalies to rescore"),
    generate_explanations: bool = Query(False, description="Whether to generate explanations"),
    db_session: AsyncSession = Depends(get_db_session),
    risk_engine: RiskEngine = Depends(get_risk_engine)
) -> RiskScoringResponse:
