...
class RiskWeights(BaseModel):
    ...
    rules_weight: float = Field(0.3, ge=0.0, le=1.0, description="Weight for rule-based scores")
...
class RiskSensitivity(BaseModel):
    ...
    medium_threshold: int = Field(60, ge=0, le=100, description="Threshold for medium risk")
...
class RiskEngine:
    ...
    def update_sensitivity(self, sensitivity: Union[RiskSensitivity, Dict[str, int]]) -> None:
    ...
    def batch_score_anomalies(self, anomalies: List[Dict[str, Any]]) -> List[RiskScoringResult]:
        ...
        for anomaly in anomalies:
            try:
                ...
                logger.error(f"Error scoring anomaly {anomaly.get('id', 'unknown')}: {str(e)}")
