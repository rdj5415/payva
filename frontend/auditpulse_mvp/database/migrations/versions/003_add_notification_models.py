...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "template_id", sa.String(100), nullable=False, unique=True, index=True
        ),
    ...
    op.add_column(
        "users", sa.Column("notification_preferences", JSONB(), nullable=True)
    )
...
def downgrade():
    ...
    op.execute(
        "DROP TRIGGER IF EXISTS update_notification_delivery_attempts_updated_at ON notification_delivery_attempts"
    )
