...
from sqlalchemy import Column, String, Text, Integer, DateTime, ForeignKey, Boolean, JSON, Enum
...
class Notification(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    template_id = Column(String(100), ForeignKey("notification_templates.template_id"), nullable=False)
    ...
    status = Column(String(50), index=True, default="pending", nullable=False)  # pending, processing, delivered, failed
...
class NotificationDeliveryAttempt(Base):
    ...
    status = Column(String(50), default="pending", nullable=False)  # pending, delivered, failed
