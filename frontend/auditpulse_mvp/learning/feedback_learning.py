...
class FeedbackLearner:
    ...
    async def process_recent_feedback(
        ...
        try:
            ...
            updated_config = await self.update_sensitivity_config(
    ...
    async def gather_feedback_data(
        ...
        rule_anomalies = [
            a for a in anomalies if a.anomaly_type == AnomalyType.RULES_BASED
        ]
        ...
        for anomaly in anomalies:
            if anomaly.feedback == FeedbackType.FALSE_POSITIVE.value:
                if (
                    ...
                    and "rule_name" in anomaly.detection_metadata
                ):
    ...
    def analyze_feedback(self, feedback_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        false_positive_rate = (
            false_positive_count / total_feedback if total_feedback > 0 else 0
        )
        true_positive_rate = (
            true_positive_count / total_feedback if total_feedback > 0 else 0
        )

        # Calculate ML-specific rates
        ml_total = len(ml_anomalies)
        ml_false_positives = sum(
            1 for a in ml_anomalies if a.feedback == FeedbackType.FALSE_POSITIVE.value
        )
        ml_true_positives = sum(
            1 for a in ml_anomalies if a.feedback == FeedbackType.TRUE_POSITIVE.value
        )
        ...
        rule_false_positives = sum(
            1 for a in rule_anomalies if a.feedback == FeedbackType.FALSE_POSITIVE.value
        )
        rule_true_positives = sum(
            1 for a in rule_anomalies if a.feedback == FeedbackType.TRUE_POSITIVE.value
        )

        rule_false_positive_rate = (
            rule_false_positives / rule_total if rule_total > 0 else 0
        )
        rule_true_positive_rate = (
            rule_true_positives / rule_total if rule_total > 0 else 0
        )
        ...
        for rule_name, count in feedback_data["rule_false_positives"].items():
            ...
            rule_anomalies_count = sum(
                ...
                if a.detection_metadata
                and a.detection_metadata.get("rule_name") == rule_name
            )
            ...
            rule_fp_analysis[rule_name] = {
                ...
                "false_positive_rate": (
                    count / rule_anomalies_count if rule_anomalies_count > 0 else 0
                ),
            }
    ...
    async def update_sensitivity_config(
        ...
        if updated_config.sensitivity_level == SensitivityLevel.CUSTOM:
            ...
            if ml_fp_rate > 0.5:
                ...
                updated_config.risk_engine.rules_score_weight = (
                    1.0 - updated_config.risk_engine.ml_score_weight
                )
    ...
    def _adjust_rule_thresholds(
        ...
        if (
            "Large Amount" in rule_analysis
            and rule_analysis["Large Amount"]["false_positive_rate"] > 0.5
        ):
        ...
        if (
            "Statistical Outlier" in rule_analysis
            and rule_analysis["Statistical Outlier"]["false_positive_rate"] > 0.5
        ):
            # Increase standard deviation threshold
            if hasattr(config.rules, "irregular_amount"):
                if (
                    config.rules.irregular_amount.parameters
                    and "std_dev_threshold" in config.rules.irregular_amount.parameters
                ):
                    config.rules.irregular_amount.parameters[
                        "std_dev_threshold"
                    ] *= 1.2  # Increase by 20%
        ...
        if (
            "Weekend Transaction" in rule_analysis
            and rule_analysis["Weekend Transaction"]["false_positive_rate"] > 0.7
        ):
        ...
        if (
            "Round Number Transaction" in rule_analysis
            and rule_analysis["Round Number Transaction"]["false_positive_rate"] > 0.7
        ):
            # If very high false positive rate, disable the rule
...
async def update_thresholds_from_feedback(
    ...
    for rule_name, counts in rule_feedback.items():
        ...
        if fp_rate > 0.7:  # High false positive rate
            ...
            adjustment = 0.2
            logger.info(
                f"High false positive rate ({fp_rate:.2f}) for rule {rule_name}"
            )
        elif fp_rate > 0.5:  # Moderate false positive rate
            # Increase threshold moderately
            adjustment = 0.1
            logger.info(
                f"Moderate false positive rate ({fp_rate:.2f}) for rule {rule_name}"
            )
        ...
        if adjustment != 0:
            try:
                ...
                await update_rule_thresholds(rule_name, adjustment, tenant_id, db)
    ...
    for model_name, counts in ml_feedback.items():
        ...
        if fp_rate > 0.7:  # High false positive rate
            ...
            logger.info(
                f"High false positive rate ({fp_rate:.2f}) for model {model_name}"
            )
        elif fp_rate > 0.5:  # Moderate false positive rate
            params = {
            ...
            logger.info(
                f"Moderate false positive rate ({fp_rate:.2f}) for model {model_name}"
            )
        elif fp_rate > 0.3:  # Low false positive rate
            params = {
            ...
            logger.info(
                f"Low false positive rate ({fp_rate:.2f}) for model {model_name}"
            )
        elif fp_rate < 0.1 and counts["true_positive"] > 0:
            # High true positive rate, decrease threshold slightly
            params = {
    ...
    if results["rules_updated"] == 0 and results["ml_models_updated"] == 0:
        results["no_action_reason"] = (
            "Insufficient feedback or no significant patterns detected"
        )
