...
class FinancialAccountService:
    ...
    async def store_plaid_access_token(
        ...
        if institution:
            ...
            institution.plaid_institution_id = (
                institution_id or institution.plaid_institution_id
            )
    ...
    async def get_accounts(self, item_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        # Get accounts
        stmt = select(FinancialAccount).where(and_(*query))
        result = await self.db.execute(stmt)
    ...
    async def get_transactions(
        ...
        if account_id:
            ...
            if institution:
                stmt = select(FinancialAccount.plaid_account_id).where(
                    FinancialAccount.user_id == self.user.id,
                    FinancialAccount.institution_id == institution.id,
                )
                result = await self.db.execute(stmt)
        ...
        count_stmt = select(func.count(FinancialTransaction.id)).where(and_(*query))
