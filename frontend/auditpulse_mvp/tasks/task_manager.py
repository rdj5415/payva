...
class TaskManager:
    ...
    async def get_task_stats(self) -> Dict[str, Any]:
        ...
        stmt = select(TaskLog.status, func.count(TaskLog.id)).group_by(TaskLog.status)
        result = await self.db.execute(stmt)
        status_counts = dict(result.all())

        # Get task counts by priority
        stmt = select(TaskLog.priority, func.count(TaskLog.id)).group_by(
            TaskLog.priority
        )
        result = await self.db.execute(stmt)
        priority_counts = {
            TaskPriority(priority).name: count for priority, count in result.all()
        }
