...
def test_validate_model(client, mock_model_manager):
    ...
    # Send request
    with patch("sklearn.metrics.accuracy_score", return_value=0.95), patch(
        "sklearn.metrics.precision_score", return_value=0.92
    ), patch("sklearn.metrics.recall_score", return_value=0.89), patch(
        "sklearn.metrics.f1_score", return_value=0.91
    ):
...
def test_predict(client, mock_model_manager):
    ...
    # Send request
    with patch("random.choice", return_value=1), patch(
        "random.uniform", return_value=0.92
    ):
