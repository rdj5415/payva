...
from auditpulse_mvp.notifications.notification_manager import NotificationManager, NotificationPriority
from auditpulse_mvp.tasks.task_manager import TaskManager
from auditpulse_mvp.database.models.user import User
from auditpulse_mvp.database.models.notification import Notification, NotificationTemplate
...
def test_create_template(client, mock_current_user, monkeypatch):
    ...
    mock_template_manager.create_template = AsyncMock(return_value={
    ...
    # Apply patch
    with patch("auditpulse_mvp.notifications.templates.TemplateManager", return_value=mock_template_manager):
        # Send request
...
def test_list_templates(client, mock_current_user, monkeypatch):
    ...
    # Mock template manager
    mock_template_manager = AsyncMock()
    mock_template_manager.list_templates = AsyncMock(return_value=[
        {
...
def test_create_notification(client, mock_current_user, mock_notification, monkeypatch):
    ...
    mock_template_manager = AsyncMock()
    mock_template_manager.get_template = AsyncMock(return_value={
        "template_id": "test_template",
        "name": "Test Template",
        "subject": "Test Subject",
        "body": "Hello, {{ name }}!",
    ...
    mock_db.query.return_value.filter.return_value.first = AsyncMock(return_value=mock_notification)
    
    # Apply patches
    with patch("auditpulse_mvp.notifications.templates.TemplateManager", return_value=mock_template_manager), \
         patch("auditpulse_mvp.notifications.notification_manager.NotificationManager", return_value=mock_manager), \
         patch("auditpulse_mvp.database.session.get_db", AsyncMock(return_value=mock_db)), \
         patch("auditpulse_mvp.tasks.task_manager.TaskManager.get_instance", MagicMock(return_value=AsyncMock())):
...
def test_get_notification_preferences(client, mock_current_user):
    ...
    assert data["notification_preferences"] == mock_current_user.notification_preferences
...
def test_update_notification_preferences(client, mock_current_user, monkeypatch):
    ...
    # Apply patch
    with patch("auditpulse_mvp.database.session.get_db", AsyncMock(return_value=mock_db)):
        # Send request
        response = client.put(
