...
class TestFeedbackLearner:
    ...
    def test_analyze_feedback(self, sample_anomalies):
        ...
        feedback_data = {
            ...
            "ml_anomalies": [a for a in sample_anomalies if a.anomaly_type == AnomalyType.ML_BASED],
            "rule_anomalies": [a for a in sample_anomalies if a.anomaly_type == AnomalyType.RULES_BASED],
    ...
    async def test_update_sensitivity_config_high_fp(
        ...
        # Arrange
        tenant_id = uuid.uuid4()
        mock_db_session.execute.return_value.scalar_one_or_none.return_value = MagicMock(spec=TenantConfiguration)
        ...
        with patch(
            ...
            # Act
            result = await learner.update_sensitivity_config(tenant_id, feedback_analysis)
    ...
    async def test_adjust_rule_thresholds(self, sample_sensitivity_config):
        ...
        assert config.rules.weekend_transaction.enabled is False  # Disabled due to high FP rate
    ...
    async def test_process_recent_feedback_success(
        ...
        with patch.object(
            learner, "gather_feedback_data", return_value={"anomalies": sample_anomalies}
        ), patch.object(
            learner, "analyze_feedback", return_value={
            ...
            learner.ml_engine, "train_model", return_value={
    ...
    async def test_process_recent_feedback_model_error(
        ...
        with patch.object(
            ...
            learner.ml_engine, "train_model", side_effect=HTTPException(
                status_code=400, detail="Not enough data"
            )
        ):
            # Act
            result = await learner.process_recent_feedback(tenant_id, days=30)
