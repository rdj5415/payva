...
class TestFeedbackLearningIntegration:
    ...
    async def test_end_to_end_learning_process(
        self, mock_db_session, test_tenant_id, sample_anomalies
    ):
    ...
    async def test_learner_process_updates_sensitivity(
        self, mock_db_session, test_tenant_id, sample_anomalies
    ):
        ...
        with patch.object(
            learner, "update_sensitivity_config", side_effect=mock_update_sensitivity
        ), patch.object(
            learner.ml_engine, "train_model", return_value={"status": "success"}
        ):
    ...
    async def test_scheduler_runs_feedback_learning(
        ...
        with patch(
            "auditpulse_mvp.learning.scheduler.get_all_tenant_ids",
            return_value=tenant_ids,
        ), patch(
            "auditpulse_mvp.learning.scheduler.get_feedback_learner",
    ...
    async def test_rule_threshold_update_integration(
        self, mock_db_session, test_tenant_id
    ):
        """Test that rule thresholds are updated correctly based on feedback."""
        # Mock the rule update function
        with patch(
            "auditpulse_mvp.rules.rule_engine.update_rule_thresholds", return_value=None
        ) as mock_update:
            # Call the actual function
            adjustment = 0.2
            rule_name = "Large Amount"

            await update_rule_thresholds(
                rule_name, adjustment, test_tenant_id, mock_db_session
            )
    ...
    async def test_ml_model_update_integration(self, test_tenant_id):
        ...
        with patch(
            "auditpulse_mvp.ml.models.update_ml_model_parameters", return_value=None
        ) as mock_update:
            # Call the actual function
...
async def test_feedback_learning_with_real_db(mock_db_session, test_tenant_id):
    ...
    for i in range(15):
        ...
        anomaly.detection_metadata = {
            "rule_name": "Large Amount" if i < 10 else "Weekend Transaction"
        }
    ...
    mock_db_session.execute.side_effect = [
        ...
        MagicMock(
            scalars=MagicMock(
                return_value=MagicMock(all=MagicMock(return_value=tenants))
            )
        ),
        # Second call: gather_feedback_data
        MagicMock(
            scalars=MagicMock(
                return_value=MagicMock(all=MagicMock(return_value=anomalies))
            )
        ),
    ...
    with patch(
        ...
        "auditpulse_mvp.learning.feedback_learning.update_ml_model_parameters"
    ) as mock_update_ml, patch(
        "auditpulse_mvp.api.api_v1.endpoints.config.get_tenant_sensitivity_config"
    ) as mock_get_config, patch(
        "auditpulse_mvp.api.api_v1.endpoints.config.get_preset_configuration"
    ) as mock_get_preset:
        ...
        mock_update_rules.assert_any_call(
            "Large Amount", 0.2, test_tenant_id, mock_db_session
        )
