...
class TestFeedbackLearningScheduler:
    ...
    async def test_init(self, mock_db_session):
        ...
        with patch(
            "auditpulse_mvp.learning.scheduler.AsyncIOScheduler"
    ...
    async def test_start_scheduler(self, mock_db_session, mock_feedback_learner):
        ...
        with patch(
            "auditpulse_mvp.learning.scheduler.get_feedback_learner",
            return_value=mock_feedback_learner,
        ):
            scheduler = FeedbackLearningScheduler(db_session=mock_db_session)
    ...
    async def test_run_feedback_learning(self, mock_db_session, mock_feedback_learner):
        ...
        with patch(
            ...
            # Assert
            assert mock_feedback_learner.process_recent_feedback.call_count == len(
                tenant_ids
            )
            for tenant_id in tenant_ids:
                mock_feedback_learner.process_recent_feedback.assert_any_call(
                    tenant_id, days=30
                )
    ...
    async def test_run_feedback_learning_with_error(self, mock_db_session):
        ...
        with patch(
            ...
            "auditpulse_mvp.learning.scheduler.logger"
        ) as mock_logger:
...
async def test_get_all_tenant_ids(mock_db_session):
    ...
    # Act
    with patch("auditpulse_mvp.learning.scheduler.select"), patch(
        "auditpulse_mvp.learning.scheduler.Tenant"
    ):
