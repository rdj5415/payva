...
class RiskWeights(BaseModel):
    ...
    def weights_must_sum_to_100(cls, v, values):
        ...
        if "rules_weight" in values:
            ...
            if not (99.99 <= total <= 100.01) and len(values) == 2:  # Only check when all 3 weights are set
    ...
    def medium_threshold_must_be_greater_than_low(cls, v, values):
        ...
        if "low_threshold" in values and v <= values["low_threshold"]:
            raise ValueError(f"Medium threshold ({v}) must be greater than low threshold ({values['low_threshold']})")
        return v
    ...
    def high_threshold_must_be_greater_than_medium(cls, v, values):
        ...
        if "medium_threshold" in values and v <= values["medium_threshold"]:
            raise ValueError(f"High threshold ({v}) must be greater than medium threshold ({values['medium_threshold']})")
        return v
...
class RiskEngine:
    ...
    async def update_config(self, config: RiskConfig) -> bool:
        ...
        try:
            ...
            logger.error(f"Error updating risk configuration for tenant {tenant_id}: {e}")
    ...
    async def evaluate_transaction(
        ...
        if config.use_gpt and flags:
            try:
                explanation = await self.gpt_engine.generate_explanation(transaction, flags)
    ...
    async def evaluate_and_store(
        ...
        try:
            ...
            if is_anomalous:
                ...
                primary_type = anomaly_types[0] if anomaly_types else AnomalyType.RULE_BASED
