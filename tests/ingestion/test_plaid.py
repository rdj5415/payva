...
@pytest.fixture
async def mock_tenant(db_session: AsyncSession, mock_tenant_id: uuid.UUID, mock_plaid_settings: dict) -> Tenant:
...
async def test_plaid_client_init():
    ...
    with patch("auditpulse_mvp.ingestion.plaid.settings") as mock_settings, \
         patch("auditpulse_mvp.ingestion.plaid.plaid.ApiClient") as MockApiClient, \
         patch("auditpulse_mvp.ingestion.plaid.plaid_api.PlaidApi") as MockPlaidApi:
...
async def test_plaid_client_get_plaid_host():
    ...
    with patch("auditpulse_mvp.ingestion.plaid.settings") as mock_settings, \
         patch("auditpulse_mvp.ingestion.plaid.plaid.ApiClient"), \
...
async def test_plaid_client_create_link_token_success():
    ...
    with patch("auditpulse_mvp.ingestion.plaid.settings") as mock_settings, \
         patch("auditpulse_mvp.ingestion.plaid.plaid.ApiClient"), \
         patch("auditpulse_mvp.ingestion.plaid.plaid_api.PlaidApi") as MockPlaidApi, \
         patch("auditpulse_mvp.ingestion.plaid.LinkTokenCreateRequest") as MockLinkTokenRequest:
        mock_settings.PLAID_CLIENT_ID = "test-client-id"
        mock_settings.PLAID_SECRET.get_secret_value.return_value = "test-secret"
...
async def test_plaid_client_create_link_token_api_error():
    ...
    with patch("auditpulse_mvp.ingestion.plaid.settings") as mock_settings, \
         patch("auditpulse_mvp.ingestion.plaid.plaid.ApiClient"), \
         patch("auditpulse_mvp.ingestion.plaid.plaid_api.PlaidApi") as MockPlaidApi, \
         patch("auditpulse_mvp.ingestion.plaid.retry", lambda f, **kwargs: f):
        mock_settings.PLAID_CLIENT_ID = "test-client-id"
        mock_settings.PLAID_SECRET.get_secret_value.return_value = "test-secret"
...
async def test_plaid_client_exchange_public_token_success():
    ...
    with patch("auditpulse_mvp.ingestion.plaid.settings") as mock_settings, \
         patch("auditpulse_mvp.ingestion.plaid.plaid.ApiClient"), \
         patch("auditpulse_mvp.ingestion.plaid.plaid_api.PlaidApi") as MockPlaidApi, \
         patch("auditpulse_mvp.ingestion.plaid.plaid.model.item_public_token_exchange_request.ItemPublicTokenExchangeRequest") as MockExchangeRequest, \
         patch("auditpulse_mvp.ingestion.plaid.ItemGetRequest") as MockItemGetRequest, \
         patch("auditpulse_mvp.ingestion.plaid.InstitutionsGetByIdRequest") as MockInstitutionRequest:
        mock_settings.PLAID_CLIENT_ID = "test-client-id"
        mock_settings.PLAID_SECRET.get_secret_value.return_value = "test-secret"
        ...
        mock_plaid_client.institutions_get_by_id.return_value = mock_institution_response
...
@pytest.mark.asyncio
async def test_plaid_client_fetch_transactions_success(mock_plaid_transaction, mock_plaid_account):
    ...
    with patch("auditpulse_mvp.ingestion.plaid.settings") as mock_settings, \
         patch("auditpulse_mvp.ingestion.plaid.plaid.ApiClient"), \
         patch("auditpulse_mvp.ingestion.plaid.plaid_api.PlaidApi") as MockPlaidApi, \
         patch("auditpulse_mvp.ingestion.plaid.TransactionsGetRequest") as MockGetRequest, \
         patch("auditpulse_mvp.ingestion.plaid.TransactionsGetRequestOptions") as MockOptions:
        mock_settings.PLAID_CLIENT_ID = "test-client-id"
        mock_settings.PLAID_SECRET.get_secret_value.return_value = "test-secret"
...
@pytest.mark.asyncio
async def test_plaid_client_fetch_transactions_pagination(mock_plaid_transaction, mock_plaid_account):
...
async def test_plaid_client_refresh_transactions_success():
    ...
    with patch("auditpulse_mvp.ingestion.plaid.settings") as mock_settings, \
         patch("auditpulse_mvp.ingestion.plaid.plaid.ApiClient"), \
         patch("auditpulse_mvp.ingestion.plaid.plaid_api.PlaidApi") as MockPlaidApi, \
         patch("auditpulse_mvp.ingestion.plaid.TransactionsRefreshRequest") as MockRefreshRequest:
        mock_settings.PLAID_CLIENT_ID = "test-client-id"
        mock_settings.PLAID_SECRET.get_secret_value.return_value = "test-secret"
...
async def test_plaid_client_fire_webhook_sandbox_success():
    ...
    with patch("auditpulse_mvp.ingestion.plaid.settings") as mock_settings, \
         patch("auditpulse_mvp.ingestion.plaid.plaid.ApiClient"), \
         patch("auditpulse_mvp.ingestion.plaid.plaid_api.PlaidApi") as MockPlaidApi, \
         patch("auditpulse_mvp.ingestion.plaid.SandboxItemFireWebhookRequest") as MockWebhookRequest:
        mock_settings.PLAID_CLIENT_ID = "test-client-id"
        mock_settings.PLAID_SECRET.get_secret_value.return_value = "test-secret"
        mock_settings.PLAID_ENVIRONMENT = "sandbox"
...
async def test_plaid_client_fire_webhook_non_sandbox():
    ...
    with patch("auditpulse_mvp.ingestion.plaid.settings") as mock_settings, \
        ...
        assert "Sandbox webhooks can only be fired in the sandbox environment" in str(excinfo.value.detail)
...
@pytest.mark.asyncio
async def test_plaid_service_get_client_for_nonexistent_tenant(db_session: AsyncSession):
...
async def test_plaid_service_get_client_for_tenant_without_plaid(
    ...
    with patch("auditpulse_mvp.ingestion.plaid.settings") as mock_settings, \
        ...
        assert excinfo.value.status_code == 400
        assert f"Plaid not configured for tenant {mock_tenant_id}" in str(excinfo.value.detail)
...
@pytest.mark.asyncio
async def test_plaid_normalize_transaction(mock_plaid_transaction: dict, mock_plaid_account: dict):
    ...
    assert normalized["amount"] == 100.50  # Should be flipped from Plaid's sign convention
...
async def test_update_tenant_plaid_settings(
    ...
    result = await db_session.execute(
        select(Tenant).where(Tenant.id == mock_tenant_id)
    )
...
async def test_handle_plaid_webhook_default_update(
    ...
    with patch.object(PlaidService, "sync_transactions") as mock_sync:
        ...
        assert (mock_sync.call_args[1]["end_date"] - mock_sync.call_args[1]["start_date"]).days <= 30
