...
async def test_evaluate_transaction(
    ...
    with patch.object(engine, "get_config", return_value=config) as mock_get_config:
        ...
        )  # Between medium (50) and high (75) thresholds

        # Check that all components were called
        mock_get_config.assert_called_once_with(mock_tenant_id)
        mock_rules_engine.flags.assert_called_once_with(
            mock_tenant_id, mock_transaction
        )
        mock_rules_engine.score.assert_called_once_with(
            mock_tenant_id, mock_transaction
        )
        mock_ml_engine.score.assert_called_once_with(mock_tenant_id, mock_transaction)
        mock_gpt_engine.generate_explanation.assert_called_once_with(
            mock_transaction, mock_rules_engine.flags.return_value
        )
...
async def test_evaluate_and_store_anomalous(
    ...
    with patch.object(
        engine, "evaluate_transaction", return_value=mock_result
    ) as mock_evaluate, patch.object(db_session, "commit") as mock_commit:
...
async def test_evaluate_and_store_not_anomalous(
    ...
    with patch.object(
        engine, "evaluate_transaction", return_value=mock_result
    ) as mock_evaluate, patch.object(db_session, "add") as mock_add, patch.object(
        db_session, "commit"
    ) as mock_commit:
...
async def test_evaluate_and_store_error(
    ...
    with patch.object(engine, "evaluate_transaction") as mock_evaluate, patch.object(
        db_session, "rollback"
    ) as mock_rollback:
